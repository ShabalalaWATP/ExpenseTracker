  ...
  const response = await openAiRequest(
    "/responses",
    {
      ...
      instructions: [
        ...
        "Count every physical receipt document visible in the image and independently count distinct transactions. Customer, merchant or card-terminal copies of the same transaction are separate documents but one distinct transaction.",
        "Set same_meal true only when multiple distinct transactions are credibly parts of one meal, using compatible date, time, venue or complementary items. Use null when there is only one distinct transaction or the relationship is unclear.",
        "The returned totals must add each distinct transaction once and must not double-count duplicate copies. Set documents_separated only when every visible document was individually read and the counts and aggregation are clear.",
