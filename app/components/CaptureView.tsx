...
export function CaptureView({
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      if (pendingExpenseId) {
        await sendReceipt(pendingExpenseId);
        return;
      }
      setStage("creating");
  ...
  async function sendReceipt(expenseId = pendingExpenseId) {
    if (!file) {
      setStage("failed");
      setError(
        "The selected photo is no longer available on this device. Retake or reselect it to finish this expense.",
      );
      return;
    }
    try {
      ...
      await uploadReceipt(expenseId, file, key);
      setStage("saved");
      setPendingExpenseId("");
      uploadKeyRef.current = "";
      await onSaved();
      ...
      setStage("failed");
      setError(caught instanceof Error ? caught.message : "The receipt upload failed.");
      requestAnimationFrame(() => errorRef.current?.focus());
    }
  }
  ...
  return (
    <div className="view page-enter capture-view">
      <ViewHeader
        eyebrow="New expense"
        title="Capture a receipt"
        detail="Photograph the whole receipt in good light, then confirm the claim details."
      />

      {error ? (
        <div ref={errorRef} tabIndex={-1}>
          <StatusMessage tone="error">
            <strong>Could not finish saving</strong>
            <p>{error}</p>
          </StatusMessage>
        </div>
      ) : null}
      {stage === "saved" ? (
        <StatusMessage tone="success">
          ...
          <p>The record and receipt upload were both acknowledged.</p>
          <button className="text-button" type="button" onClick={reset}>Capture another</button>
        </StatusMessage>
      ) : null}

      <form className="capture-layout" onSubmit={handleSubmit} noValidate>
        <section className="photo-well" aria-label="Receipt photo">
          {preview ? (
            <div className="receipt-preview">
              <Image src={preview} alt="Selected receipt preview" fill unoptimized />
            </div>
          ) : (
            <div className="camera-prompt">
              <span aria-hidden="true">＋</span>
              <strong>Add receipt photo</strong>
              <p>Camera or Photo Library</p>
            </div>
          )}
        ...
        <section className="review-form" aria-labelledby="review-heading">
          <div className="section-heading"><div><p className="eyebrow">Review</p><h2 id="review-heading">Expense details</h2></div><span className="required-key">* Required</span></div>
          <div className="form-grid">
            ...
            <Field label="Meal context"><select value={mealContext} onChange={(event) => setMealContext(event.target.value as MealContext)}><option value="">Not labelled</option><option value="breakfast">Breakfast</option><option value="lunch">Lunch</option><option value="dinner">Dinner</option><option value="snack">Snack</option><option value="mixed">Mixed</option></select></Field>
          ...
          <div className="form-actions">
            ...
            <button className="primary-button" type="submit" disabled={busy || stage === "saved"}>
              {stage === "creating" ? "Creating expense…" : stage === "uploading" ? "Uploading receipt…" : "Save expense"}
            </button>
          </div>
